...
class D4PGAgent():
    def __init__(self,batch_size,state_size,action_size,num_nodes,replaybuffer):
        ...
        self.z_torch = torch.from_numpy(self.z_dist).to(torch.float32).to(device)
        
        
        
        self.actor_local = Actor(self.state_size,action_size).to(device)
        self.critic_local = Critic(self.state_size,self.action_size, self.num_nodes).to(device)
        
        self.actor_target = Actor(self.state_size,self.action_size).to(device)
        self.critic_target = Critic(self.state_size,self.action_size, self.num_nodes).to(device)
        self.lr_actor=1e-3
        self.lr_critic=1e-3
    ...
    def learn(self):
        ...
        next_prob = F.softmax(self.critic_target(next_state,action_next),dim=1)
        
        q_next = reward.view(-1,1) + (self.gamma * self.z_torch.view(1,-1) * (1 - done.view(-1,1)) )
        q_next = torch.clamp(q_next,self.vmin,self.vmax)
        b = (q_next - self.vmin)/ self.deltaz
        l = torch.floor(b)
        u = torch.ceil(b)
        dml = (u - b + (l == u).float()) * next_prob
        dmu = (b - l) * next_prob
        target_prob = torch.from_numpy(np.zeros((self.batch_size,self.num_nodes))).to(torch.float32).to(device)
        for i in range(self.batch_size):
            target_prob[i].index_add_(0,l[i].long(),dml[i])
            target_prob[i].index_add_(0,u[i].long(),dmu[i])
        
        prob = self.critic_local(state,action)
        prob = F.log_softmax(prob,dim=1)
        
        critic_loss = (-prob * target_prob).sum(-1).mean()
